...
| `KeyError: 'ContainerConfig'` | docker-compose v1 incompatible with newer Docker | Use `docker run` with `--env-file` + `-v` + `-p` instead of `docker-compose up -d` |
| `.env` changes not picked up | Env vars baked at container creation, not on restart | Must recreate container: `docker stop && docker rm && docker run --env-file .env ...` |
